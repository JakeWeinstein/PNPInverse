...
\item Real-time fit diagnostics: live curve plot with target/current/best and per-evaluation trajectories.
\end{enumerate}
...
\((\mathrm{mA}/\mathrm{cm}^2)\), not molar flux. The two are linked by Faraday's law.

If \(F_i\) is the \emph{integrated} species flux through the electrode boundary
\((\mathrm{mol}/\mathrm{s})\), then the corresponding flux density is
\[
N_i = \frac{F_i}{A_{\mathrm{electrode}}}
\qquad [\mathrm{mol}\,\mathrm{m}^{-2}\,\mathrm{s}^{-1}],
\]
where \(A_{\mathrm{electrode}}\) is the electrode boundary measure.

For a species involving \(n_i\) electrons per molecule, the partial current density is
\[
j_i = \pm n_i F N_i
\qquad [\mathrm{A}\,\mathrm{m}^{-2}],
\]
with \(F=96485.3329\,\mathrm{C/mol}\) and sign chosen by electrochemical
convention (e.g., cathodic reduction often plotted negative).

Unit conversion to the experimental plotting convention is
\[
j_i(\mathrm{mA}/\mathrm{cm}^2) = 0.1\, j_i(\mathrm{A}/\mathrm{m}^2).
\]

For peroxide production, \(n_i=2\), so once species flux is computed at each
\(\phi_{\mathrm{applied}}\), a peroxide current-density polarization curve is obtained by direct scaling.
This means fitting flux-vs-voltage curves is equivalent (up to fixed constants and area)
to fitting current-density-vs-voltage curves.
...
\item Boundary flux outputs can be converted directly to partial current density, enabling direct comparison to experimentally measured polarization curves.
...
\item In noiseless synthetic tests, recovered \(\kappa\) is close to truth; with 5\% RMS noise, solutions remain feasible but can shift to different local minima.
\item Live plotting materially improved debugging visibility during fitting.
\end{itemize}
