"""Joint (k0, alpha) inference from peroxide current -- EXTENDED range.
...
Usage (from PNPInverse/ directory)::
    ...
    python scripts/inference/Infer_BVJoint_charged_peroxide_current_extended.py
"""
...
def main() -> None:
    ...
    output_dir = os.path.join("StudyResults", "bv_joint_peroxide_current_charged_extended")
    ...
    request = BVFluxCurveInferenceRequest(
        ...
        observable_title="Joint (k0, alpha) from peroxide current (extended)",
        control_mode="joint",
    ...
    print(f"\n=== Joint Inference from Peroxide Current (Extended Range) ===")
