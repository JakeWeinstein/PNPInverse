"""Adjoint-gradient full (k0, alpha, steric_a) inference from I-V curve data.
...
This is the most challenging inference mode: 8 parameters from 10 I-V points.
The steric parameters may not be fully identifiable when combined with k0/alpha.

Usage (from PNPInverse/ directory)::

    python scripts/inference/Infer_BVFull_charged_from_current_density_curve.py
"""
...
def main() -> None:
    ...
    # Initial guesses: deliberate perturbation from truth
    initial_k0_guess = [0.005, 0.0005]     # ~4x and ~10x off
    initial_alpha_guess = [0.4, 0.3]        # ~0.2 off
    initial_steric_a_guess = [0.1, 0.1, 0.1, 0.1]  # 2x off
    ...
    output_dir = os.path.join("StudyResults", "bv_full_charged")
    ...
    request = BVFluxCurveInferenceRequest(
        ...
        observable_title="Full (k0+alpha+steric) inference",
        control_mode="full",
    ...
    # Final summary
    print("\n" + "=" * 80)
    print("  FULL (k0 + alpha + steric) INFERENCE SUMMARY")
    print("=" * 80)
