...
Recovers [k0_1, k0_2] for the 2-species coupled BV model (O2 + H2O2)
from a synthetic current-density vs. overpotential curve using the same
adjoint-gradient + L-BFGS-B framework as the Robin kappa inference.
...
All 7 convergence strategies:
  1. Voltage continuation with warm-start
  2. Inner time-stepping / pseudo-transient
  3. Exponent clipping at +/-50
  4. Concentration floor eps_c=1e-12
  5. Dirichlet-decoupled BV exponent (use_eta_in_bv=True)
  6. l2 linesearch with lambda_max=0.5
  7. MUMPS direct solver with auto-scaling

Usage (from PNPInverse/ directory)::

    python scripts/inference/Infer_BVk0_from_current_density_curve.py
"""
...
# ---------------------------------------------------------------------------
# SNES options (all 7 convergence strategies)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    # Initial guess — deliberately wrong (10x too high for both)
    ...
    output_dir = os.path.join("StudyResults", "bv_k0_inference")
    ...
    request = BVFluxCurveInferenceRequest(
        ...
        observable_title="BV k0 inference from I-V curve",
        k0_lower=1e-8,
        k0_upper=100.0,
        mesh_Nx=4,
        mesh_Ny=200,
