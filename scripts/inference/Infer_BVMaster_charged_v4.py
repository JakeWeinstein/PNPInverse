"""Master inference protocol v4 for BV kinetics (charged system) -- SPEED OPTIMIZED.

Full 4-species charged system: O2, H2O2, H+, ClO4- (z=[0,0,+1,-1]).

Speed optimizations vs v3:
    1. Shared forward solutions: secondary observable reuses cached ICs from primary
       (implemented in FluxCurve/bv_curve_eval.py, ~35% speedup on multi-obs evals)
    2. Moderately tighter optimizer convergence: P2 maxiter 40->30, P3 maxiter 30->25,
       ftol 1e-12->1e-10 (10-15% speedup without accuracy loss)
    3. Predictor validation: bad extrapolations caught before SNES
       (implemented in FluxCurve/bv_point_solve.py, ~10% speedup)

REVERTED optimizations (hurt accuracy by >2%):
    - Skip Phase 1: causes >50% k0_1 error (alpha warm-start is essential)
    - Fewer voltage points (7-pt P2, 10-pt P3): causes >50% k0_1 error
...
Usage (from PNPInverse/ directory)::
    ...
    python scripts/inference/Infer_BVMaster_charged_v4.py
"""
...
def main() -> None:
    # ===================================================================
    # Voltage grids (OPTIMIZED: fewer points in P2 and P3)
    # ===================================================================
    # Phase 1: 12-point symmetric [-20, +5] (same as v3)
    ...
    # Phase 2: 10-point SHALLOW cathodic [-1, -13] (same as v3)
    ...
    # Phase 3: 15-point FULL cathodic [-1, -46.5] (same as v3)
    ...
    # Deliberately wrong initial guesses (same as v3)
    initial_k0_guess = [0.005, 0.0005]
    initial_alpha_guess = [0.4, 0.3]

    observable_scale = -I_SCALE
    base_output = os.path.join("StudyResults", "master_inference_v4")
    ...
    print(f"\n{'#'*70}")
    print(f"  MASTER INFERENCE PROTOCOL v4 (SPEED OPTIMIZED)")
    print(f"  Changes vs v3: shared forward solutions, predictor validation,")
    print(f"  moderately tighter convergence (P2:30, P3:25 iters)")
    print(f"  REVERTED: skip-P1 and fewer-pts (both cause >50% k0 error)")
    ...
    #          7 points, maxiter=25, gtol=1e-5, ftol=1e-8
    # ===================================================================
    print(f"\n{'='*70}")
    ...
    #          Warm-start all 4 params from Phase 2.
    #          10 points, maxiter=20, gtol=1e-5, ftol=1e-8
    # ===================================================================
    print(f"\n{'='*70}")
    ...
    # ===================================================================
    print(f"\n{'#'*90}")
    print(f"  MASTER INFERENCE v4 SUMMARY (SPEED OPTIMIZED)")
    print(f"{'#'*90}")
    print(f"  True k0:    {true_k0}")
    print(f"  True alpha: {true_alpha}")
    ...
    # Save comparison CSV
    csv_path = os.path.join(base_output, "master_comparison_v4.csv")
