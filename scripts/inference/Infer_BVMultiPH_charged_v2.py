...
# ---------------------------------------------------------------------------
# Fix 2: SNES tuning -- backtracking linesearch + reduced maxlambda
# ---------------------------------------------------------------------------
SNES_OPTS = {
    "snes_type":                 "newtonls",
    "snes_max_it":               300,
    ...
    "snes_rtol":                 1e-10,
    "snes_stol":                 1e-12,
    "snes_linesearch_type":      "l2",       # Keep l2 (bt was too conservative)
    "snes_linesearch_maxlambda": 0.3,        # Fix 2: reduced from 0.5
...
def _make_bv_solver_params(eta_hat: float, dt: float, t_end: float,
                           c_hp_hat: float = C_HP_HAT) -> SolverParams:
    """Build SolverParams for 4-species charged BV with variable H+ conc."""
    params = dict(SNES_OPTS)
    # Fix 6: Enable softplus concentration regularization
    params["bv_convergence"] = {
        "clip_exponent": True, "exponent_clip": 50.0,
        "regularize_concentration": True, "conc_floor": 1e-12,
        "use_eta_in_bv": True,
        "softplus_regularization": True,     # Fix 6: enable softplus
    }
    params["nondim"] = {
        "enabled": True,
...
def main() -> None:
    ...
    base_sp = _make_bv_solver_params(eta_hat=0.0, dt=dt, t_end=t_end, c_hp_hat=C_HP_HAT)
