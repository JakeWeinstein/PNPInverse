"""Adjoint-gradient steric 'a' inference -- EXTENDED voltage range.
...
Usage (from PNPInverse/ directory)::
    ...
    python scripts/inference/Infer_BVSteric_charged_extended.py
"""
...
# ---------------------------------------------------------------------------
# Physical constants and scales (identical to other charged inference scripts)
# ---------------------------------------------------------------------------
...
def _make_bv_solver_params(eta_hat: float, dt: float, t_end: float) -> SolverParams:
    ...
    return SolverParams.from_list([
        ...
        TRUE_STERIC_A,  # params[6] = a_vals (true steric values for target generation)
        eta_hat,
...
def main() -> None:
    # Extended 15-point placement spanning full voltage range
    eta_values = np.array([
        -1.0, -2.0, -3.0,           # onset
    ...
    output_dir = os.path.join("StudyResults", "bv_steric_charged_extended")
    ...
    request = BVFluxCurveInferenceRequest(
        ...
        observable_title="Steric 'a' inference (extended range, charged 4-species)",
        control_mode="steric",
        ...
        # Steric inference settings -- reduced upper bound for robustness
        true_steric_a=TRUE_STERIC_A,
        initial_steric_a_guess=initial_steric_a_guess,
        steric_a_lower=0.001,
        steric_a_upper=0.15,
        # Bridge points: auto-insert for gaps > 3.0
        max_eta_gap=3.0,
        # Mesh
    ...
    # Final summary
    print("\n" + "=" * 70)
    print("  STERIC INFERENCE SUMMARY (EXTENDED RANGE)")
    print("=" * 70)
    print(f"  Voltage range: eta_hat in [{eta_values[0]}, {eta_values[-1]}]")
    print(f"  Inference points: {len(eta_values)}")
