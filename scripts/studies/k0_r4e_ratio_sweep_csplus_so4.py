"""K0_R4e/K0_R2e ratio sweep — find the regime where Butler shape emerges.

At Pass A defaults (K0_R4e = K0_R2e), R_4e dominates by ~20 orders of
magnitude via the (E_eq_R4e − E_eq_R2e)/V_T exponential and saturates
the H⁺ mass-transport floor — IV plateau, no peroxide selectivity.
The Mangan/Ruggiero deck shows substantial peroxide on a carbon
catalyst (CMK-3); matching it requires kinetic suppression of the
4e pathway, modeled here as K0_R4e ≪ K0_R2e.

This driver sweeps a ratio K0_R4e = K0_R2e × ratio over the values

    {1.0, 1e-6, 1e-12, 1e-18, 1e-24, 1e-30}

(the 1e-30 endpoint comes from the Phase 5γ GATE v2 memory: that
factor was needed to escape the H⁺ floor at eta_R4e ≈ 28). For each
ratio it:

  1. Builds a fresh anchor at V_RHE = +0.55 V via
     :func:`solve_anchor_with_continuation`.
  2. Walks the 8-voltage grid {+0.10, …, +0.80} V via
     :func:`solve_grid_with_anchor` — the same path Pass A uses.
  3. Captures cd (total) and pc (gross R_2e, deck-aligned) per V.
  4. Writes per-ratio iv-curve JSON to
     ``StudyResults/.../k0_r4e_ratio_sweep/ratio_<r>/`` and a
     combined ``summary.json`` at the sweep root.

Anchor failures at very small ratios are tolerated: the ratio is
recorded as ``anchor_failed`` and the sweep continues.

Usage::

    python -u scripts/studies/k0_r4e_ratio_sweep_csplus_so4.py
"""
...
# Ratios K0_R4e/K0_R2e to sweep. 1.0 reproduces Pass A baseline; the
# log-spaced descent down to 1e-30 covers the regime where the H⁺
# mass-transport floor releases its grip on R_4e (per GATE v2 memory).
K0_R4E_RATIOS = (1.0, 1e-6, 1e-12, 1e-18, 1e-24, 1e-30)
...
def _ratio_label(ratio: float) -> str:
    """Filename-safe label for a ratio (e.g. 1e-12 → 'ratio_1e-12')."""
    if ratio == 1.0:
        return "ratio_1e+00"
...
def _run_one_ratio(ratio: float, *, mesh) -> dict:
    """Anchor + walk for a single K0_R4e/K0_R2e ratio."""
    ...
    for i, v in enumerate(V_RHE_GRID):
        ...
        cd_s = (f"{cd_arr[i]:+.3e}"
                if np.isfinite(cd_arr[i]) else "    nan ")
        pc_s = (f"{pc_arr[i]:+.3e}"
                if np.isfinite(pc_arr[i]) else "    nan ")
        print(f"    V_RHE={v:+.3f}  {tag}  cd={cd_s} pc={pc_s}")
...
def main() -> int:
    ...
    print("=" * 78)
    print(f"  K0_R4e/K0_R2e ratio sweep — multi-ion (Cs⁺/SO₄²⁻ + parallel-2e/4e)")
    print("=" * 78)
    ...
    for ratio in K0_R4E_RATIOS:
        ...
        with open(out_subdir / "pass_a_iv_curve.json", "w") as f:
