"""Phase D bridge — Cs⁺/SO₄²⁻ variant for slide-15 electrolyte match.

Same flags as the K⁺ bridge (`_phase_D_bridge_no_hydrolysis.py`):
   * Stern = 0.20 F/m² (two-stage anchor 0.10→0.20)
   * V10B kinetics; k0_R4e_factor = 1e-14
   * L_eff = 16 μm
   * enable_cation_hydrolysis = False, enable_water_ionization = False

Differences:
   * 3sp + bikerman counterions (Cs⁺ via Boltzmann, SO₄²⁻ via Boltzmann)
   * multi_ion_enabled = True
   * H_SPECIES_INDEX = 2 (same as K⁺ variant; H+ is index 2 in 3sp stack)

V grid: V_RHE ∈ [-0.50, +0.55], dV = 0.05 (22 pts) — matches slide-15 band.
"""
...
OUT_DIR = Path(_ROOT) / "StudyResults" / "phase6b_step10_phase_D_no_hydrolysis_bridge_Cs"
    ...
    rxns = [
        {
            ...
            "anodic_species": 1,
            "c_ref": 1.0,
            "stoichiometry": [-1, +1, -2],   # 3sp ordering: O2, H2O2, H+
            "n_electrons": 2,
            "reversible": True,
...
def main() -> int:
    ...
    print("=" * 78)
    print("  Phase D bridge — Cs⁺/SO₄²⁻ (slide-15 electrolyte), no hydrolysis no kw")
    print("=" * 78)
    ...
    report = {
        "label": "phase_D_bridge_no_hydrolysis_no_kw_Cs",
        "config": {
            "species_preset": "THREE_SPECIES_LOGC_BOLTZMANN",
    ...
    if finite.any():
        ...
        print(f"  V_RHE most-cathodic pc: V = {v_arr[i_peak]:+.3f} V, "
