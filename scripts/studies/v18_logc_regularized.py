"""V18 k0 inference with Tikhonov regularization.

Adds a soft prior on log(k0) to break the k0-alpha ridge degeneracy.
The prior represents literature/EIS-measured k0 with uncertainty.

J_total = data_misfit + lambda * (log(k0) - log(k0_prior))^2

A small lambda (0.1-1.0) is usually enough to break the degeneracy
without biasing far from what the data supports.
"""
...
def main():
    ...
    noise = rng.normal(0, 0.02, size=target_cd.shape) * np.abs(target_cd)
    noisy = target_cd + noise

    # Prior: k0_prior = true value (simulating a literature measurement).
    # With real data, this would come from EIS or Tafel analysis.
    log_k0_prior = np.log(K0_HAT_R1)

    # Reduced sweep: skip lam=0 (already confirmed ridge-limited in main inference test).
    # Test lam=0.1 (moderate prior) and lam=0.01 (weak prior) only.
    results = []
    for lam in [0.01, 0.1]:
        ...
        def objective(x):
            ...
            if not np.all(np.isfinite(cds)):
                ...
                diff = cds - noisy
            J_data = 0.5 * np.sum(diff**2)
            J_prior = lam * (log_k0 - log_k0_prior)**2
            J = J_data + J_prior
            print(f"  [{eval_count[0]}] k0={k0:.3e} alpha={alpha:.4f} J_data={J_data:.3e} J_prior={J_prior:.3e}", flush=True)
            return J

        x0 = [np.log(K0_HAT_R1 * 1.2), ALPHA_R1 * 0.9]
        t0 = time.time()
        # Reduced maxiter: with regularization the bowl is well-conditioned
        # so NM converges faster (no ridge to walk along)
        res = minimize(objective, x0, method="Nelder-Mead",
                      options={"xatol": 1e-4, "fatol": 1e-10, "maxiter": 20})
        k0_rec = np.exp(res.x[0])
        alpha_rec = res.x[1]
        k0_err = 100 * (k0_rec - K0_HAT_R1) / K0_HAT_R1
        alpha_err = 100 * (alpha_rec - ALPHA_R1) / ALPHA_R1
        results.append((lam, k0_rec, alpha_rec, k0_err, alpha_err, res.fun, eval_count[0], time.time()-t0))
        print(f"  Recovered: k0={k0_rec:.3e} ({k0_err:+.1f}%), alpha={alpha_rec:.4f} ({alpha_err:+.1f}%)")

    print(f"\n{'='*60}\nREGULARIZATION SWEEP (2% noise)\n{'='*60}")
    print(f"  {'lambda':>7}  {'k0_rec':>11}  {'k0 err':>8}  {'alpha':>7}  {'α err':>7}  {'J':>10}  {'evals':>5}")
    for lam, k0, alpha, k0_err, alpha_err, J, n, t in results:
        print(f"  {lam:>7.3f}  {k0:>11.3e}  {k0_err:>+7.1f}%  {alpha:>7.4f}  {alpha_err:>+6.1f}%  {J:>10.2e}  {n:>5}")
