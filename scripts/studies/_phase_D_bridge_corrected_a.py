"""Phase D bridge with PHYSICAL per-species a_nondim — K⁺/SO₄²⁻ variant.

Identical to ``_phase_D_bridge_no_hydrolysis.py`` except the
``a_vals_hat`` entries on the dynamic species O₂, H₂O₂, H⁺ are
swapped from the placeholder ``A_DEFAULT = 0.01`` (≈ r 14.9 Å —
physically meaningless) to hard-sphere radii from the literature:

    O₂   r = 1.7 Å (Marcus / kinetic diameter ≈ 3.4 Å diameter)
    H₂O₂ r = 2.0 Å
    H⁺   r = 2.8 Å (H₃O⁺ Stokes radius)

The counterions (K⁺, SO₄²⁻) already use physical radii (2.3 Å,
2.4 Å) per Linsey 2025 ACS-CATL deck slide 13.

This run is paired with the un-corrected K⁺ bridge so we can read
off how much of the Phase D plateau topology is caused by the
artificially-tight H⁺ Bikerman cap (c_max ∝ 1/a).
"""
...
OUT_DIR = Path(_ROOT) / "StudyResults" / "phase6b_step10_phase_D_bridge_corrected_a"

# ---------------------------------------------------------------------------
# Physical a_nondim values for dynamic species
# ---------------------------------------------------------------------------
# a_phys = (4/3)·π·r³·N_A m³/mol;  a_nondim = a_phys · C_SCALE.
# C_SCALE = 1.2 mol/m³ (Ruggiero baseline, see scripts/_bv_common.py).
_C_SCALE = 1.2
_N_A = 6.02214076e23
...
# r values: O₂ 1.7 Å, H₂O₂ 2.0 Å (Marcus ≈ 1.75–2.0 Å), H₃O⁺ 2.8 Å (Stokes).
A_O2_PHYSICAL = _a_nondim_from_radius_m(1.70e-10)     # ≈ 1.487e-5
A_H2O2_PHYSICAL = _a_nondim_from_radius_m(2.00e-10)   # ≈ 2.422e-5
A_HP_PHYSICAL = _a_nondim_from_radius_m(2.80e-10)     # ≈ 6.645e-5
...
def _make_sp(*, stern_capacitance_f_m2: float):
    from scripts._bv_common import (
        ALPHA_R2E, ALPHA_R4E,
        A_KPLUS_HAT,
        C_HP_HAT, C_KPLUS_HAT, C_O2_HAT, H2O2_SEED_NONDIM,
        D_H2O2_HAT, D_HP_HAT, D_KPLUS_HAT, D_O2_HAT,
    ...
    # Custom 4-species config: identical to FOUR_SPECIES_LOGC_DYNAMIC_K2SO4
    # but with realistic per-species a_vals_hat for O₂/H₂O₂/H⁺.
    species = SpeciesConfig(
        n_species=4,
        z_vals=[0, 0, 1, 1],
        d_vals_hat=[D_O2_HAT, D_H2O2_HAT, D_HP_HAT, D_KPLUS_HAT],
        a_vals_hat=[A_O2_PHYSICAL, A_H2O2_PHYSICAL, A_HP_PHYSICAL, A_KPLUS_HAT],
...
def main() -> int:
    ...
    print("=" * 78)
    print("  Phase D bridge — K⁺/SO₄²⁻ + PHYSICAL per-species a_nondim")
    print("=" * 78)
    ...
    report = {
        "label": "phase_D_bridge_corrected_a_K",
        "config": {
            "species_preset": "FOUR_SPECIES_LOGC_DYNAMIC_K2SO4 (custom a_vals)",
            "a_vals_hat_dynamic": {
                "O2": A_O2_PHYSICAL,
