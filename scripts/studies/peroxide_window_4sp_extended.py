"""4sp dynamic — extended V grid, clip=50, with/without Stern.

Replaces the earlier ``peroxide_window_4sp_quick.py`` test which used
``clip=100`` (the wrong value for cathodic V — see
``docs/clip_observable_investigation.md`` §3 and §5.4).

The extended V grid gives the C+D warm-walk plenty of cold anchors in
the cathodic regime where 4sp + linear_phi is known to converge
(``tests/test_solver_equivalence.py``).  We then watch how far the
warm-walk extends into the peroxide window — that tells us whether
finite Stern is necessary for 4sp to reach V ≥ +0.68 V.

Two passes:

    (A)  4sp + linear_phi + clip=50 + no Stern    — control
    (B)  4sp + linear_phi + clip=50 + C_S=0.10    — Stern aid

V_RHE grid (15 voltages): cathodic anchors → peroxide window:
    [-0.5, -0.3, -0.1, 0.0, +0.1, +0.3, +0.5, +0.55, +0.60, +0.65,
     +0.66, +0.68, +0.70, +0.75, +1.00]

Output: StudyResults/peroxide_window_4sp_extended/
"""
...
PASSES = (
    ("4sp_no_stern_clip50",      None),
    ("4sp_stern_0p10_clip50",    0.10),
)
MESH_NY = 200
EXPONENT_CLIP = 50.0      # production default per CLAUDE.md hard-rule #4 (clip=100 causes cathodic exp overflow at V=-0.5)
U_CLAMP = 100.0
N_SUBSTEPS_WARM = 8
BISECT_DEPTH_WARM = 5
STERIC_CAP = 100.0
# Optional positional override: `python ... debye_boltzmann` runs with
# the analytical IC.  Default linear_phi matches the prior pass.
INITIALIZER = sys.argv[1] if len(sys.argv) > 1 else "linear_phi"
# Optional second positional override: `python ... debye_boltzmann logc_muh`
# selects the muh formulation backend.  Default "logc" matches production.
FORMULATION = sys.argv[2] if len(sys.argv) > 2 else "logc"
_FORM_TAG = "" if FORMULATION == "logc" else f"_{FORMULATION}"
OUT_SUBDIR = f"peroxide_window_4sp_extended_{INITIALIZER}{_FORM_TAG}"
...
def _make_plot(reports: list[dict], png_path: str) -> Optional[str]:
    ...
    colors = {"4sp_no_stern_clip50": "k", "4sp_stern_0p10_clip50": "C0"}
    markers = {"4sp_no_stern_clip50": "o", "4sp_stern_0p10_clip50": "s"}
    ...
    ax_cd.set_title(
        "4sp dynamic ClO4- — extended V grid, clip=50, ±Stern\n"
        f"(Ny={MESH_NY}, initializer={INITIALIZER})"
    )
...
def main() -> None:
    ...
    print("=" * 78)
    print("  4sp dynamic — extended V grid, clip=50, with/without Stern")
    print("=" * 78)
    print(f"  V_RHE grid (n={len(V_TEST)}): {list(V_TEST)}")
    print(f"  passes        = {[p[0] for p in PASSES]}")
    print(f"  mesh_Ny       = {MESH_NY}")
    print(f"  exponent_clip = {EXPONENT_CLIP} (production default)")
    ...
    for label, cs in PASSES:
        ...
        for i, v in enumerate(V_TEST):
            ...
            print(f"    V={v:+.3f}  ok={ok}  cd={cd_s}  pc={pc_s}  "
                  f"method={method_s}  c_ClO4={c_clo4_s}  "
    ...
    for r in reports:
        # Find the highest converged voltage
        v_arr = r["v_rhe"]
        ok = r["converged"]
        last_v = max((v_arr[i] for i in range(len(v_arr)) if ok[i]),
                     default=None)
        last_str = f"V={last_v:+.3f}" if last_v is not None else "(none)"
        print(f"    {r['label']:30s}  {r['n_converged']}/{r['n_total']}  "
              f"max-V={last_str}")
    print("=" * 78)
