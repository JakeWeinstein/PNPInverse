...
from Forward.bv_solver.forms_logc import (
    build_context_logc as build_context,
    build_forms_logc as build_forms,
    set_initial_conditions_logc as set_initial_conditions,
)
...
# Log-c transform: u_i = ln(c_i). c = exp(u) > 0 always.
# H2O2 seed (1e-4) avoids log(0) = -inf singularity; set_initial_conditions_logc
# uses u_H2O2_init = log(1e-4) = -9.2 instead of log(1e-20) = -46.
H2O2_SEED = 1e-4
...
def test_point(V_RHE, z_steps=20):
    ...
    # In log-c mode: U.dat[i] stores u = log(c). Transform to c.
    u_min = [float(U.dat[i].data_ro.min()) for i in range(n)]
    u_max = [float(U.dat[i].data_ro.max()) for i in range(n)]
    c_min = [np.exp(u) for u in u_min]
    c_max = [np.exp(u) for u in u_max]
    print(f"  z=0: ok={ok_z0}, cd={cd_z0:.6f}, c_min={[f'{c:.2e}' for c in c_min]}, c_max={[f'{c:.2e}' for c in c_max]}")

    # Log-c physics validation (F1 impossible by construction)
    ...
    for i, name in enumerate(species_names):
        if c_bulk[i] > 1e-20 and c_max[i] > c_bulk[i] * (5.0 if z_nominal[i] != 0 else 1.20):
            _warns.append(f"W2: {name} exceeds bulk, max={c_max[i]:.4g} vs bulk={c_bulk[i]:.4g}")
        if c_bulk[i] > 1e-20 and z_nominal[i] > 0:
            # cation depletion check (simple)
            if c_min[i] < c_bulk[i] * 1e-3:
                _warns.append(f"W5: {name} depleted, min={c_min[i]:.4g}")
    # F6: H2O2 > O2 bulk
    if n >= 2 and c_max[1] > c_bulk[0] * 1.05:
        _fails.append(f"F6: H2O2 exceeds O2 bulk, max={c_max[1]:.4g}")
    if _fails:
        print(f"    [validation] FAILURES: {'; '.join(_fails)}")
    if _warns:
        print(f"    [validation] warnings: {'; '.join(_warns)}")
    ...
    with adj.stop_annotating():
        for z_val in np.linspace(0, 1, z_steps+1)[1:]:
            ...
            if ok_z or steps_z > 0:
                ...
                u_min_vals = [float(U.dat[i].data_ro.min()) for i in range(n)]
                u_max_vals = [float(U.dat[i].data_ro.max()) for i in range(n)]
                c_min_vals = [np.exp(u) for u in u_min_vals]
                c_max_vals = [np.exp(u) for u in u_max_vals]
                if z_val in [0.05, 0.5, 0.75, 1.0] or not ok_z:
                    print(f"  z={z_val:.3f}: ok={ok_z}, cd={cd_z:.6f}, c_min={[f'{c:.2e}' for c in c_min_vals]}, c_max={[f'{c:.2e}' for c in c_max_vals]}")
            else:
                for src, dst in zip(U_ckpt, U.dat): dst.data[:] = src
                Up.assign(U)
                c_min_vals = [np.exp(float(U.dat[i].data_ro.min())) for i in range(n)]
                print(f"  z={z_val:.3f}: FAILED ({msg_z}), c_min={[f'{c:.2e}' for c in c_min_vals]}")
