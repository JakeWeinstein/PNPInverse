...
# EXPERIMENT (Option A): seed only the INITIAL CONDITION for H2O2 to a
# small positive value (applied after set_initial_conditions), keeping the
# bulk Dirichlet BC at c=0. Steady state physics unchanged (set by BCs),
# but transient starts from a numerically-safe positive state instead of
# c=0 where CG1 oscillations around zero are unconstrained.
H2O2_IC_SEED = 1e-7  # nondim, just above the regularization floor (1e-8)

THREE_SPECIES_Z = [0, 0, 1]
THREE_SPECIES_D = [D_O2_HAT, D_H2O2_HAT, D_HP_HAT]
THREE_SPECIES_A = [A_DEFAULT, A_DEFAULT, A_DEFAULT]
THREE_SPECIES_C0 = [C_O2_HAT, C_H2O2_HAT, C_HP_HAT]  # bulk/BC = 0 for H2O2
...
def test_point(V_RHE, z_steps=20):
    ...
    with adj.stop_annotating():
        ...
        # Override IC for H2O2 (species index 1) with a linear profile that
        # matches the Dirichlet BC at bulk (0 at y=1) and has H2O2_IC_SEED
        # at the electrode (y=0). This avoids IC/BC conflict at the boundary.
        coords = fd.SpatialCoordinate(mesh)
        y = coords[1] if mesh.geometric_dimension() >= 2 else coords[0]
        ctx["U_prev"].sub(1).interpolate(fd.Constant(H2O2_IC_SEED) * (1.0 - y))
