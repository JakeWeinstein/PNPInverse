"""Master inference protocol v16 for BV kinetics.

Builds on v15's streamlined 3-phase architecture (S->P1->P2) and integrates
the unified grid charge continuation framework for target generation and
voltage extension.  Replaces per-point sequential loops with
solve_grid_with_charge_continuation() for both target gen and P2 cache
population.
...
Three-phase pipeline:
    ...
    Target gen + voltage extension: unified grid charge continuation

Usage (from PNPInverse/ directory)::

    # Full pipeline
    python scripts/Inference/Infer_BVMaster_charged_v16.py

    # Surrogate-only (fast, ~10s)
    python scripts/Inference/Infer_BVMaster_charged_v16.py --no-pde

    # Use RBF instead of NN ensemble
    python scripts/Inference/Infer_BVMaster_charged_v16.py --model-type rbf

    # Custom charge continuation parameters
    python scripts/Inference/Infer_BVMaster_charged_v16.py --max-eta-gap 4.0 --charge-steps 15
"""
...
def _target_cache_path(phi_applied_values, observable_scale, max_eta_gap, charge_steps):
    ...
    Includes method=unified_grid to avoid cache collision with v13/v15.
    """
    import hashlib
    ...
    parts = [
        ...
        # Unified grid charge continuation method marker + parameters
...
def _solve_clean_targets_unified(phi_applied_values, observable_scale,
                                 charge_steps, max_eta_gap=3.0):
    """Generate noise-free targets using unified grid charge continuation."""
    import firedrake as fd
    import pyadjoint as adj
    from Forward.bv_solver import make_graded_rectangle_mesh
    from Forward.bv_solver import solve_grid_with_charge_continuation
    ...
    clean_cd = np.zeros(n_eta)
    clean_pc = np.zeros(n_eta)

    print(f"  [unified-grid targets] Solving {n_eta} voltage points "
          f"(max_eta_gap={max_eta_gap}, charge_steps={charge_steps})")

    def _extract_observables(orig_idx, phi_applied, ctx):
        """per_point_callback: extract CD and PC from live context."""
    ...
    if not result.all_converged():
        import warnings
        partial = result.partial_points()
        warnings.warn(f"Target gen: {len(partial)} points not fully converged")
...
def _generate_targets_with_charge_cont(phi_applied_values, observable_scale,
                                       ...
                                       max_eta_gap, charge_steps):
    """Generate target I-V curves using unified grid charge continuation.
    ...
    if os.path.exists(cache_path):
        try:
            ...
            if (clean_cd.shape[0] == len(phi_applied_values)
                ...
                clean_cd, clean_pc = _solve_clean_targets_unified(
                    phi_applied_values, observable_scale, charge_steps, max_eta_gap)
...
def _extend_voltage_cache_for_p2_unified(
    ...
    *,
    ...
    """Pre-solve all P2 voltages using unified grid continuation.

    NOTE: solver_params here use warm_k0/warm_alpha from P1's result,
    NOT the true kinetic parameters. This matches v15 behavior.
    ...
    if not result.all_converged():
        partial = result.partial_points()
        pts_str = ", ".join(
            f"eta={p.phi_applied:+.2f} z={p.achieved_z_factor:.3f}" for p in partial
        )
        warnings.warn(f"Extension: partial-z points: {pts_str}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="BV Master Inference v16 (Unified Grid Continuation + Multistart + PDE)"
    )
    ...
    parser.add_argument("--max-eta-gap", type=float, default=3.0,
                        help="Max eta gap before bridge point insertion (default: 3.0)")
    parser.add_argument("--charge-steps", type=int, default=10,
                        help="Charge ramping steps for target generation (default: 10)")
    parser.add_argument("--eta-steps", type=int, default=None,
                        help="DEPRECATED: ignored in v16, use --max-eta-gap instead")
    ...
    if args.eta_steps is not None:
        import warnings
        warnings.warn("--eta-steps is deprecated in v16 and ignored. Use --max-eta-gap.")
    ...
    if len(pde_eta) < len(all_eta):
        ...
        print(f"  [v16] --max-anodic-eta={args.max_anodic_eta}: "
    ...
    base_output = os.path.join("StudyResults", "master_inference_v16")
    ...
    print(f"\n{'#'*70}")
    print(f"  MASTER INFERENCE PROTOCOL v16")
    ...
    print(f"  Charge cont:         max_eta_gap={args.max_eta_gap}, charge_steps={args.charge_steps}")
    if not args.no_pde:
    ...
    targets = _generate_targets_with_charge_cont(
        ...
        args.max_eta_gap, args.charge_steps,
    )
    ...
    if not args.no_pde:
        ...
        print(f"\n  [v16] Shared parallel pool: {n_pde_workers} workers")
        ...
        if args.skip_p1:
            print(f"\n  [v16] --skip-p1: skipping P1, P2 warm-starts from {warm_source_label}")
            ...
            _clear_caches()
            t_ext = time.time()
            n_cached = _extend_voltage_cache_for_p2_unified(
                pde_eta, p2_warm_k0, p2_warm_alpha,
                charge_steps=args.charge_steps, max_eta_gap=args.max_eta_gap,
            )
            ext_time = time.time() - t_ext
            print(f"  [v16] Extension: {n_cached}/{len(pde_eta)} cached ({ext_time:.1f}s)")
        else:
            print(f"\n{'='*70}")
        ...
        close_parallel_pool()
        _clear_caches()
        print(f"  [v16] Shared parallel pool closed")
    ...
    # ===================================================================
    print(f"\n{'#'*95}")
    print(f"  MASTER INFERENCE v16 SUMMARY")
    print(f"  (Charge Continuation + Multistart + PDE Full Range)")
    print(f"{'#'*95}")
    print(f"  Model type:        {args.model_type}")
    print(f"  True k0:           {true_k0}")
    ...
    print(f"\n  {'='*80}")
    print(f"  v13 / v16 COMPARISON:")
    print(f"  {'='*80}")
    print(f"  {'Metric':<25} {'v13':>10} {'v16':>10}")
    print(f"  {'-'*48}")
    ...
    csv_path = os.path.join(base_output, "master_comparison_v16.csv")
