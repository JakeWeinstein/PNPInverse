# Round 3 counterreply — k2so4-dual-series-fit

All 11 accepted.

**Re 1 (penalty poisons L-BFGS-B):** Accept — replaced with an exact
raise-and-restart policy: in-evaluation retry ladder (re-walk from
nearest converged neighbor with max_inserts+2, then half-step V
insertion); if still failed the evaluation RAISES and never returns
(f, g); outer driver restarts L-BFGS-B from the last valid
checkpointed iterate with fresh Hessian and the offending step
halved (temporarily shrunk bounds box); >3 restarts ⇒ stop and
diagnose. No constant penalties or stale gradients ever reach the
optimizer. FD points must themselves converge or the check is
re-centered.

**Re 2 (17-pt surrogate):** Accept — final L-BFGS-B POLISH on the
full 30-bin-center objective; FD gates, profiles, sensitivities,
and the reported θ all use that final objective; the adaptive grid
is now explicitly only a warm-up.

**Re 3 (relative-only FD tolerances):** Accept — FD in scaled
optimizer variables with combined tolerance |g_adj − g_fd| ≤
max(0.05·|g_fd|, 1e-3·max_k|g_k|), plus a directional-derivative
check along a random unit vector in scaled space; all FD points
converged and inside bounds. Ledger-cd equality likewise gets a
combined tolerance (1e-12 abs nondim, 1e-10 rel).

**Re 4 (N refit algebra):** Accept your better option — the ring
series enters the objective RAW (baseline-corrected j_ring,
mA/cm²_ring, anodic-positive) with σ_ring independent of N; the
collection model moves to the model side: j_ring_model =
−pc_model·N·A_d/A_r. N variants become pure model-side refits with
no target/σ regeneration. The disk-equivalent conversion survives
for plots/slide-15 comparability only.

**Re 5 (O₂ protocol missing):** Accept — data ask added
(gas/temperature/O₂-saturation protocol); if unconfirmed, c_O₂
refits {1.0, 1.3} mol/m³ around the production 1.2 and kinetic
parameters labeled transport-conditional, with the k0–c_O₂ partial
degeneracy noted explicitly.

**Re 6 (ring baseline):** Accept — per-cycle ring baseline = median
raw j_ring over the predeclared no-H₂O₂ region (disk non-cathodic),
subtracted; baseline spread propagated into σ_ring in quadrature;
bounded ring-offset nuisance pre-registered as a default-OFF refit
sensitivity; canonical Sel%/n_e use baseline-corrected I_r.

**Re 7 (refit vs rescore):** Accept — explicit table added: REFIT
for window edge, OCP variants, L_eff, N, bulk c_H, c_O₂, weight
swaps/unweighted/Huber, ring-offset nuisance; RESCORE only for pure
ablations (water-2e off, water-4e off, acid-on, Kw 0.5×). Refits
warm-started; spreads reported as structural error bars; any spread
exceeding profile widths makes the affected parameter conditional.

**Re 8 (parameter profiles ≠ partition identifiability):** Accept —
added derived-quantity profiles for the partition itself (4e current
fraction at plateau V and integrated over the window): lower
envelope of the (g, J) cloud over all checkpointed evals + 3–5
constrained refits at fixed g (quadratic penalty, penalty-weight
sweep checked), under both dual and pc-only objectives. The paper
claim cites parameter AND derived-quantity profile widths.

**Re 9 (0.47 V uncertainty wider than the cal bracket):** Accept —
OCP refit set widened to {0.969, 0.994, 1.044, 1.069} (±50 mV on
the 0.47 component beyond the calibration bracket); θ is labeled
OCP-conditional until the 0.47 V component is confirmed, and that
confirmation is now BLOCKING for paper-grade lock (Stage 5), not
just for absolute-potential claims.

**Re 10 (σ vs SEM):** Accept — σ declared a conservative
single-observation predictive-error scale (|Δ|/√2, not the SEM
|Δ|/2 of the fitted mean); absolute reduced-χ² explicitly not
interpreted; only relative comparisons on the fixed vector.

**Re 11 (brittle n_e gate):** Accept — demoted to a thresholded
diagnostic: evaluated only where |I_d| > 10·σ_floor_disk and
I_r > 10·σ_floor_ring; excursions outside [1.8, 4.2] trigger
STOP-AND-DIAGNOSE (documented investigation), not automatic
rejection.

## Updated artifact

Changes this round: exact raise-and-restart failed-eval policy
(discipline section); bin-center final polish + all reporting on the
final objective; scaled abs+rel FD tolerances + directional check;
raw-ring objective with model-side N; ring baseline correction step;
O₂ protocol ask + c_O₂ refits + transport-conditional labels; refit
vs rescore table; derived-quantity (partition) profiles; widened OCP
refit set + OCP-conditional θ + lock-blocking confirmation; σ/SEM
framing; n_e thresholded diagnostic; risk table rows 8–9.

---

# Phase 7.2 — Dual-series (disk + ring) fit to the K₂SO₄ pH 6.39 RRDE data

**Status: in GPT critique session 43 (R1: 18 issues, R2: 11 issues —
all accepted; this version incorporates both rounds).**
...
8. Hydrodynamics + protocol asks (R1#13, R2#5): rotation rate NOT
   in file → assume 1600 rpm (L_eff = 15.4 µm central; Stage 4
   ...
   degeneracy noted explicitly). DATA ASKS: rpm; gas/temperature/
   O₂-saturation protocol; catalyst identity (CMK-3 vs GC);
   ring hold potential + ring calibration; sibling pH files.
...
**n_e diagnostic (R2#11, demoted from hard gate):** recomputed
n_e(V) evaluated only where |I_d| > 10·σ_floor_disk AND
I_r > 10·σ_floor_ring; excursions outside [1.8, 4.2] ⇒ STOP AND
DIAGNOSE (documented investigation), not automatic rejection.
...
- `calibration/phase7_wls.py` → `score_dual_series`: optimization
  ...
  vector) logged at every evaluation; per-series reduced χ²
  surfaced; failed-eval behavior per the discipline section; fast
...
- **Grid discipline (R1#14, R2#2):** fit ITERATIONS on a
  ...
  outright.
- **FD gate (R2#3):** central-FD h-convergence on all 4 controls
  against the final objective, in SCALED optimizer variables;
  combined tolerance |g_adj − g_fd| ≤ max(0.05·|g_fd|,
  1e-3·max_k|g_k|); plus one directional-derivative check along a
  random unit vector in scaled space; every FD point converged and
  inside bounds (else re-centered).
...
- **Identifiability protocol (R1#9, R2#8):** (i) BFGS
  ...
  **derived-quantity profiles for the partition itself**: 4e
  current fraction at the plateau V and integrated over the window
  — via (a) the lower envelope of the (g, J) cloud over ALL
  checkpointed evaluations, and (b) 3–5 constrained refits at
  fixed g (quadratic penalty on g, penalty-weight sweep checked).
...
- Lock: bin-center curves both conditions, residual panels, full
  ...
  **Paper-grade lock blocked on the 0.47 V OCP confirmation**
  (θ stays OCP-conditional otherwise).
