# Round 5 counterreply — k2so4-dual-series-fit

All 4 accepted.

**Re 1 (re-included bins must trigger refit):** Accept — the
discipline now reads: a re-included bin re-runs the bin-center
POLISH and the downstream FD/profile gates on the expanded mask;
the old θ is never assumed optimal for a changed data vector.

**Re 2 (raw Tafel xlsx gates transferability):** Accept — the lock
table row for cross-condition transferability now requires catalyst
identity AND the raw Tafel xlsx; until the xlsx lands, the claim is
labeled "digitized-slide-conditional".

**Re 3 (risk-table wording):** Accept — risk row 6 now says
"per-series standardized residual score" and "pseudo-Huber".

**Re 4 (Huber kink vs adjoint):** Accept — the robustness refit uses
pseudo-Huber (smooth, adjoint-compatible), stated in the refit table.

## Updated artifact

Only the four edits above; no other changes this round. The full
current artifact follows.

---

# Phase 7.2 — Dual-series (disk + ring) fit to the K₂SO₄ pH 6.39 RRDE data

**Status: in GPT critique session 43 (R1: 18, R2: 11, R3: 7, R4: 4
issues — all accepted; this version incorporates all four rounds).**
...
- Fit grid and data mask FROZEN before optimization (Stage 1
  ...
  RETRIED at the polished θ; it is then either RE-INCLUDED — in
  which case the bin-center polish AND the downstream FD/profile
  gates are re-run on the expanded mask (the old θ is not assumed
  optimal for the changed data vector) — or excluded with a
...
- **Refit vs rescore table (R2#7):**
  ...
  | Weights: w_r ×2/×½, unweighted, pseudo-Huber (smooth — adjoint-compatible; vanilla Huber's kink is not) | REFIT |
...
- **Lock tiers (R3#3):**
  ...
  - **Paper-grade claims, gated per ask:**
    ...
    | cross-condition transferability (vs slide-15 Cs⁺) | catalyst identity AND raw Tafel xlsx (until then the claim is "digitized-slide-conditional") |
...
| 6 | One series dominates objective | per-series standardized residual score every eval; weight-swap/unweighted/pseudo-Huber REFITS |
