...
def add_percent_noise(
    values: Sequence[float], noise_percent: float, *, seed: Optional[int] = None
) -> np.ndarray:
...
def configure_bv_solver_params(
    ...
    *,
    phi_applied: float,
    k0_values: Optional[Sequence[float]] = None,
    ...
    Sets ``params[7]`` and ``params[9]`` to ``phi_applied``.  If *k0_values*
    ...
    If *a_values* is given, sets the Bikerman steric parameter (``params[6]``).
    """
    params = deep_copy_solver_params(base_solver_params)
    ...
    if k0_values is not None:
        ...
        if reactions is not None and isinstance(reactions, list):
            for j, rxn in enumerate(reactions):
                if j < len(k0_list):
                    rxn["k0"] = k0_list[j]
        # Legacy per-species path: only update if lengths match
        if "k0" in bv_cfg:
            existing = bv_cfg["k0"]
    ...
    if a_values is not None:
        # Steric a_vals are in params[6] (the a_vals slot)
        n_species = int(params[0])
...
def sweep_phi_applied_steady_bv_flux(
    ...
    *,
    ...
    with _maybe_stop_annotating():
        ...
        jac = fd.derivative(F_res, U)
        problem = fd.NonlinearVariationalProblem(F_res, U, bcs=bcs, J=jac)
        solver = fd.NonlinearVariationalSolver(problem, solver_parameters=params0[10])
