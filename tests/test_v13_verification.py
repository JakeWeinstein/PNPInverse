...
# ---------------------------------------------------------------------------
# Test 5: Surrogate vs PDE consistency
# ---------------------------------------------------------------------------

@pytest.mark.slow
class TestSurrogateVsPDEConsistency:
    """Surrogate and PDE predictions should agree within ~15% at the same parameters.

    Mathematical property tested:
        The NN surrogate is trained on PDE data. At the training parameters
        and voltage grid, the surrogate should reproduce the PDE output
        within its approximation error (typically 1-5% for well-trained
        models, up to 15% at parameter space boundaries).

    This test checks consistency at the true parameters AND at 2 perturbed
    parameter sets to catch systematic biases.

    Tolerance:
        - 15% relative error per voltage point: generous to account for
          surrogate approximation error, especially at extreme overpotentials.
        - Uses a small voltage grid (5 points) for PDE speed.
    """

    def test_surrogate_matches_pde(self, nn_ensemble):
        """Surrogate predictions agree with PDE within 15% relative error."""
        try:
            import firedrake  # noqa: F401
        except ImportError:
            pytest.skip("Firedrake not available")
        ...
        # Use a subset of the surrogate's voltage grid
        surr_phi = nn_ensemble.phi_applied
        # Pick 5 moderately cathodic points from the surrogate grid
        cathodic_phi = surr_phi[surr_phi < -2.0]
        if len(cathodic_phi) < 5:
            pytest.skip("Not enough cathodic points in surrogate grid")
        step = max(1, len(cathodic_phi) // 5)
        test_phi = cathodic_phi[::step][:5]

        # Find corresponding indices in the full surrogate grid
        surr_indices = []
        for phi_val in test_phi:
            idx = np.argmin(np.abs(surr_phi - phi_val))
            surr_indices.append(idx)
        surr_indices = np.array(surr_indices)
        ...
        # Test at true parameters plus 2 perturbed sets
        param_sets = [
            (K0_HAT_R1, K0_HAT_R2, ALPHA_R1, ALPHA_R2, "true"),
            (K0_HAT_R1 * 2.0, K0_HAT_R2, ALPHA_R1 * 0.9, ALPHA_R2, "perturbed_1"),
            (K0_HAT_R1, K0_HAT_R2 * 3.0, ALPHA_R1, ALPHA_R2 * 1.1, "perturbed_2"),
        ]

        for k0_1, k0_2, a1, a2, label in param_sets:
            # Surrogate prediction at the test voltages
            surr_pred = nn_ensemble.predict(k0_1, k0_2, a1, a2)
            surr_cd = surr_pred["current_density"][surr_indices]
            ...
            _clear_caches()
            dummy_target = np.zeros_like(test_phi, dtype=float)
            points = solve_bv_curve_points_with_warmstart(
                base_solver_params=base_sp,
                steady=steady,
                phi_applied_values=test_phi,
                target_flux=dummy_target,
                k0_values=[k0_1, k0_2],
                blob_initial_condition=False,
                fail_penalty=1e9,
            ...
            # Compare: 15% relative error tolerance
            for j in range(len(test_phi)):
                if abs(pde_cd[j]) > 1e-10:
                    relerr = abs(surr_cd[j] - pde_cd[j]) / abs(pde_cd[j])
                    assert relerr < 0.15, (
                        f"[{label}] Surrogate vs PDE mismatch at phi={test_phi[j]:.1f}: "
                        f"surrogate={surr_cd[j]:.6e}, PDE={pde_cd[j]:.6e}, "
                        f"relative error={relerr:.2%}"
                    )
