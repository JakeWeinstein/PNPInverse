...
- **INV-01** (TestParameterRecovery): Recover known parameters from
  surrogate-generated synthetic data at 0%, 1%, 2%, 5% noise levels with
...
- **INV-03** (TestMultistartBasin): Multistart LHS grid search with
  polish -- >50% of top-20 candidates should recover true parameters.
...
class TestParameterRecovery:
    """Recover known BV kinetics parameters from surrogate-generated synthetic data.

    Uses the surrogate-only inference pipeline (S1 alpha-only + S2 joint
    L-BFGS-B) to recover 4 parameters at multiple noise levels.
...
class TestGradientConsistencyPDE:
    ...
    def test_pde_adjoint_vs_fd(self):
        ...
        def _pde_cd_at_params(k0_1, k0_2, alpha_1, alpha_2):
            """Run PDE solve and return current density array."""
...
class TestMultistartBasin:
    """Multistart LHS grid search should converge to the true minimum.
