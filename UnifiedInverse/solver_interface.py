...
def as_species_list(values: Any, n_species: int, name: str) -> List[float]:
    ...
    """
    # Scalar inputs are broadcast to all species for convenience.
    ...
    # At this point we have a fully validated per-species list.
    return out
...
def deep_copy_solver_params(solver_params: Sequence[Any]) -> SolverParams:
    ...
    """
    # The solver-parameter object mixes lists and dicts, so shallow copy is not enough.
...
def extract_solution_vectors(U_final: Any, n_species: int) -> Tuple[List[np.ndarray], np.ndarray]:
    ...
    """
    # By convention, concentrations are components [0..n_species-1]
    # and electric potential is component n_species.
...
class ForwardSolverAdapter:
    ...
    def from_module_path(
        ...
        ----------
        module_path:
            Python import path, for example ``"Utils.forsolve"``.
        solve_function_name:
        ...
        """
        # Import once and validate the expected adapter contract up front.
        ...
        # Allow modules to use different solve names (e.g. forsolve_robin).
    ...
    def run_forward(
        ...
        *,
        ...
        """
        # Standard forward pipeline used throughout unified inverse workflows.
